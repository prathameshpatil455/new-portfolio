...
const MultiInquiryDropDown = ({ selectedStatuses, onStatusesChange }) => {
    const statusOptions = [
        { value: 1, label: 'Open' },
        { value: 2, label: 'InProgress' },
        { value: 3, label: 'Actioned' },
    ];
    ...
    const handleChange = (event) => {
        ...
        const updatedStatuses = typeof value === 'string' ? value.split(',') : value;
            ...
            >
                {statusOptions.map((status) => (
                    <MenuItem key={status.value} value={status.label}>
                        <Checkbox checked={localSelectedStatuses.indexOf(status.label) > -1} />
...
MultiInquiryDropDown.propTypes = {
    selectedStatuses: PropTypes.array.isRequired,
    handleChange: PropTypes.func.isRequired
};

export default MultiInquiryDropDown;
